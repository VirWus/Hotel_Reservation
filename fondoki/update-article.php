...
if(!empty($nom) && !empty($checkin) && !empty($city) && !empty($lat) && !empty($len) && !empty($prix) && !empty($stars) && !empty($adress) && !empty($descr) && !empty($trapurl) ) {
    ...
    if($ext == "jpg" || $ext == "png") {
     ...
     $file = "http://192.168.43.215/fondoki/".$file;
      ...
      $query = mysqli_query($connect,"UPDATE hotels set name='$nom',stars='$st',price='$p',cityname='$city',adress='$adress',tripurl='$trapurl',lat='$la',len='$le',descr='$descr',checkin='$checkin',photo='$file' where id='$id'");
